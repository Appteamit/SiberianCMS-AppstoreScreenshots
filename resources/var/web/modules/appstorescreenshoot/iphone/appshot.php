...
class appShot {
	...
	private function manipulateImage() {
		...
		$imageCheck = $_FILES['file']['name'][0];
			...
			//	$img58->save("appshots/$randInt/5.8/$imageCount-5.8.jpg", 90);
				...
				$img65 = new abeautifulsite\SimpleImage("appshots/$randInt/upload_$image_name");
				$img65->resize(1242, 2688);
				$img65->save("appshots/$randInt/6.5/$imageCount-6.5.jpg", 90);
	...
	private function generateZip() {
		...
		//	array_push($imageFiles, "appshots/$randInt/5.8/$imageCount-5.8.jpg");
			array_push($imageFiles, "appshots/$randInt/6.5/$imageCount-6.5.jpg");
		...
		$zip = new ZipArchive();
		$zip->open("appshots/iPhone_".$randInt.".zip", ZipArchive::CREATE);
	...
	private function emailUser() {
		...
		$subject = "AppShots IPhone Generated";

		$body = "https://$host/app/local/modules/AppstoreScreenshoot/resources/var/web/modules/appstorescreenshoot/iphone/appshots/iPhone_".$randInt.".zip";
